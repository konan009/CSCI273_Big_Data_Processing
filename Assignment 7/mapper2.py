...
# Regex that match any words including that has apostrophe / This regex excludes the format above
# Example Word: it's, called, brother's, brother
regex_for_count_words = r"[a-zA-Z]+(?:\'[a-zA-Z]+)?"
...
for line in sys.stdin:
    ...
    # Use this variable for the number of words in a line
    wordcount = len(re.findall(regex_for_count_words, line))

    if len(line_info) > 0 :
        info = line_info[0]
        # Explode the array
        info = info.split(":")
        # The first number is the book number
        current_book = info[0]
